          ...
          <div class='card shadow' style={{ maxwidth: '30rem' }}>
            <div class='card-body'>
              ...
              <h6 class='card-subtitle mb-2 text-muted'>
                Posted on <Moment format='DD-MMM-YYYY'>{post.date}</Moment>{' '}
              </h6>
              <p class='card-text'>{post.text}</p>
              {showbuttons ? (
                <Fragment>
                  <span className='btn' onClick={() => likePost(post._id)}>
                    <FontAwesomeIcon icon={faThumbsUp} />{' '}
                    {post.likes.length > 0 && post.likes.length}{' '}
                  </span>
                  <span className='btn' onClick={() => unlikePost(post._id)}>
                  ...
                  {register.user && register.user._id === post.user ? (
                    <button
